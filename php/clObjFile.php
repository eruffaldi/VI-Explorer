...
class clObjFile {
  ...
  private function initNames() {
    ...
    $this->m_propNames[46]='Enumeration';
    $this->m_propNames[111]='SomeColor';
 ...
 if ($autoClose) {
   ...
   if ($ArgLen > 6)
   {
     $this->setError('Error - Property length - >6  @ '. $pos);
   }
   else if ($ArgLen == 6)
   {
         ...
         switch($ob["id"])
         {
          case 46:
            ...
            { 
                $n = ord($rvalue[0]);
                $q = substr($rvalue,1,$n);
                $rvalue = substr($rvalue,$n+1);
                array_push($pvalues,$q);
            }
            $pvalue = htmlspecialchars(json_encode($pvalues));

            break;
          default:
             $pvalue = '#'. bin2hex($ob['value']);
            break;
         }
    	  $tmp .= $space .'<prop type="'. $this->getPropertyNameById($ob['id']) .'" value="' . $pvalue .'" />'. $nl; //'" pos="'. $ob['pos'] .
    	  break;
        
       default:
       $tmp .= $space .'<unknownentity type='. $ob['_type'] .' />'. $nl;
     }
